...
def create_distance_lines(x_dists=[10, 20, 30], y_range=(-10, 10), z_pos=-1.5):
    """
    x_dists: 선을 그을 거리 목록 (예: 10m, 20m, 30m)
    y_range: 선의 좌우 길이 (예: -10m ~ 10m)
    z_pos: 선의 높이 (라이다 센서 기준 바닥 높이, 보통 -1.5 ~ -1.7)
    """
    ...
    for i, x in enumerate(x_dists):
        # 선의 시작점과 끝점 (x는 고정, y는 좌우)
        p1 = [x, y_range[0], z_pos]
        p2 = [x, y_range[1], z_pos]
        points.append(p1)
        points.append(p2)
...
def draw_scenes_with_lines(points, ref_boxes=None, ref_scores=None, ref_labels=None):
    ...
    # 포인트 색상 (Intensity가 있으면 활용, 없으면 회색)
    if points.shape[1] > 3:
        import matplotlib.pyplot as plt
        # Intensity 정규화 (보통 0~1 사이로 가정하거나 데이터에 맞춰 조정 필요)
        intensity = points[:, 3]
        intensity = (intensity - intensity.min()) / (intensity.max() - intensity.min() + 1e-6)
        # Viridis 컬러맵 적용
        c_map = plt.get_cmap('viridis')
        pts.colors = open3d.utility.Vector3dVector(c_map(intensity)[:, :3])
    else:
        pts.paint_uniform_color([0.5, 0.5, 0.5]) # 회색

    vis.add_geometry(pts)

    # 2. 박스 추가 (기존 OpenPCDet 유틸리티 활용)
    if ref_boxes is not None:
        V.draw_box(vis, ref_boxes, (0, 1, 0), ref_labels, ref_scores)

    # 3. [핵심] 거리 선 추가 (10m, 20m, 30m)
    # y_range는 좌우 시야각에 맞춰 조절하세요 (예: -20 ~ 20)
    distance_lines = create_distance_lines(x_dists=[10, 20, 30], y_range=(-20, 20), z_pos=-2.0)
    vis.add_geometry(distance_lines)
...
def main():
    ...
    with torch.no_grad():
        for idx, data_dict in enumerate(demo_dataset):
            ...
            frame_id = Path(file_path).stem  # 경로에서 확장자 떼고 이름만 추출 (예: 000008)
            
            logger.info(f'Visualized sample index: \t{idx + 1}')
            logger.info(f'Current Frame ID: \t{frame_id}')  # <-- 터미널에 출력
            ...
            # )
            if OPEN3D_FLAG:
                draw_scenes_with_lines(
                    points=data_dict['points'][:, 1:].cpu().numpy(),
                    ref_boxes=pred_dicts[0]['pred_boxes'].cpu().numpy(),    # .cpu().numpy() 추가
                    ref_scores=pred_dicts[0]['pred_scores'].cpu().numpy(),  # .cpu().numpy() 추가
                    ref_labels=pred_dicts[0]['pred_labels'].cpu().numpy()   # .cpu().numpy() 추가
                )
            else:
                # Mayavi Fallback (기존 유지)
                V.draw_scenes(
                    points=data_dict['points'][:, 1:], ref_boxes=pred_dicts[0]['pred_boxes'],
                    ref_scores=pred_dicts[0]['pred_scores'], ref_labels=pred_dicts[0]['pred_labels']
                )
