...
def parse_config():
    ...
    try:
        # 절대 경로를 상대 경로로 변환
        relative_path = cfg_path.relative_to(Path.cwd() / 'tools' / 'cfgs')
        cfg.EXP_GROUP_PATH = str(relative_path.parent)
    except (ValueError, IndexError):
        # 예외 발생 시, 파일의 부모 디렉토리 이름만 사용
        cfg.EXP_GROUP_PATH = str(cfg_path.parent.name)
