...
class A2D2Dataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.a2d2_infos = []
        self.include_a2d2_data(self.mode)

    def include_a2d2_data(self, mode):
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        from .a2d2_object_eval_python import eval as a2d2_eval
    ...
    def __len__(self):
        if self._merge_all_iters_to_one_epoch:
            return len(self.a2d2_infos) * self.total_epochs
...
def create_a2d2_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = A2D2Dataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
    train_split, val_split = 'train', 'val'

    train_filename = save_path / ('a2d2_infos_%s.pkl' % train_split)
    val_filename = save_path / ('a2d2_infos_%s.pkl' % val_split)
    trainval_filename = save_path / 'a2d2_infos_trainval.pkl'
    test_filename = save_path / 'a2d2_infos_test.pkl'

    print('---------------Start to generate data infos---------------')

    dataset.set_split(train_split)
    a2d2_infos_train = dataset.get_infos(num_workers=workers, has_label=True, count_inside_pts=True)
    with open(train_filename, 'wb') as f:
        pickle.dump(a2d2_infos_train, f)
    print('a2d2 info train file is saved to %s' % train_filename)

    dataset.set_split(val_split)
    a2d2_infos_val = dataset.get_infos(num_workers=workers, has_label=True, count_inside_pts=True)
    with open(val_filename, 'wb') as f:
        pickle.dump(a2d2_infos_val, f)
    print('a2d2 info val file is saved to %s' % val_filename)

    with open(trainval_filename, 'wb') as f:
        pickle.dump(a2d2_infos_train + a2d2_infos_val, f)
    print('a2d2 info trainval file is saved to %s' % trainval_filename)

    dataset.set_split('test')
    a2d2_infos_test = dataset.get_infos(num_workers=workers, has_label=False, count_inside_pts=False)
    with open(test_filename, 'wb') as f:
        pickle.dump(a2d2_infos_test, f)
    print('a2d2 info test file is saved to %s' % test_filename)
